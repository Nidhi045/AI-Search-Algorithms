...
def oracle_cost_heuristic_page():
    st.title("Oracle Cost Heuristic Search")
    ...
    if input_method == "Manual Input":
        ...
        if st.session_state.nodes:
            ...
            for node in st.session_state.nodes:
                heuristic_value = st.number_input(f"Enter heuristic for {node}:", min_value=0, key=f"heuristic_{node}")
        ...
        if st.button("Generate Random Graph"):
            ...
            st.session_state.edges = {node: {} for node in st.session_state.nodes}  
            st.session_state.heuristics = {node: random.randint(1, 20) for node in st.session_state.nodes}   
    ...
    if st.session_state.input_complete:
        ...
        if st.session_state.start_node and st.session_state.goal_node:
            if st.button("Find Path"):
                paths = oracle_cost_heuristic_search(st.session_state.edges, st.session_state.start_node, st.session_state.goal_node, oracle_value, st.session_state.heuristics)
                if paths:
                    for path in paths:
...
def oracle_cost_heuristic_search(edges, start, goal, oracle, heuristics):
    queue = PriorityQueue()
    queue.put((0, [start]))   
    explored = set()
    results = []
    ...
    while not queue.empty():
        ...
        for neighbor, edge_cost in edges[current_node].items():
            if neighbor not in explored and (cost + edge_cost) <= oracle:
                new_path = path + [neighbor]
                new_cost = cost + edge_cost + heuristics[neighbor]  # Add heuristic to cost
